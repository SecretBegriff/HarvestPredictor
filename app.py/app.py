...
def get_dashboard_data():
    try:
        ...
        # Esta consulta obtiene la ÚLTIMA lectura de CADA planta
        query = """
        ...
        SELECT 
            ...
            'Salon 212' as location,  -- ¡OJO! Tu BD no tiene "Location". Lo estoy hardcodeando.
            'Active' as sensor_status -- ¡OJO! Tu BD no tiene "Status". Lo estoy hardcodeando.
        FROM RankedReadings rr
        WHERE rr.rn = 1
        ORDER BY rr.plant_id;
        """
...
# Ruta para obtener TODAS las plantas registradas
...
def get_plants():
    try:
        ...
        # Obtenemos las plantas y el nombre de su tipo
        query = """
        SELECT p.id, p.planting_date, pt.name as plant_name
        FROM plants p
        JOIN plant_types pt ON p.plant_type_id = pt.id
        ORDER BY p.id
        """
        ...
        for result in results:
            if result.get('planting_date'):
                result['planting_date'] = result['planting_date'].isoformat().split('T')[0] # Formato YYYY-MM-DD
